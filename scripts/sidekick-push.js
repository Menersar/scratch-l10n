...
const guiStrings = parseGuiSourceSctrings();
const vmMessages = parseVmSourceSctrings();
const allStrings = {
    ...guiStrings,
    ...vmMessages
};
...
const sidekickStrings = {};
for (const [id, message] of Object.entries(allStrings)) {
    if (id.startsWith('gui.')) {
        sidekickStrings[id] = message;
    }
}
...
const push = async () => {
    try {
        ...
        await txPush(PROJECT, RESOURCE, sidekickStrings);
    } catch (error) {
        console.error(error);
        process.exit(1);
    }
