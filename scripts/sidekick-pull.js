...
import {supportedLocales, scratchToTransifex} from './sidekick-supported-locales';
...
const pullDesktop = async () => {
    const desktop = pathUtil.join(__dirname, '../../scratch-desktop');
    if (!isDirectorySync(desktop)) {
        console.log('Skipping desktop; could not find scratch-desktop.');
        return;
    }
